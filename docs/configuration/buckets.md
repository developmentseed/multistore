...
> [!NOTE]
> Requires the `azure` feature flag on `multistore`. Enabled by default in the server runtime, not available in CF Workers.
...
> [!NOTE]
> Requires the `gcp` feature flag on `multistore`. Enabled by default in the server runtime, not available in CF Workers.
