...
Only traits whose wasm implementations use `!Send` types need `MaybeSend + MaybeSync`: `ProxyBackend`, `RouteHandler`, `BackendAuth`, `HttpExchange`, and `CredentialExchange`. Other traits like `BucketRegistry` and `CredentialRegistry` use plain `Send + Sync`.
