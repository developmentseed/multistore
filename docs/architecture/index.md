...
|-----------|-------|---------------|
| [Proxy Handler](./request-lifecycle) | `multistore` | Dispatch operations via presigned URLs, LIST, or multipart |
...
| [STS Handler](/auth/proxy-auth#oidcsts-temporary-credentials) | `multistore-sts` | OIDC token exchange, credential minting |
| [OIDC Provider](/auth/backend-auth#oidc-backend-auth) | `multistore-oidc-provider` | Self-signed JWT minting, backend credential exchange |
